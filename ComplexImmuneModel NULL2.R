...
PopuDynamic<-function(t,Nl,l){
       ...
       #SwTo<-function(l){return((1+Diffac*(median(LAll)-l))/length(LAll))}#switch preference
        ...
        # 
         ...
         #Problistic version 
        ImKill<-function(Nl){#immune dection and kill,threshold version
                VecPro=rexp(length(l),rate = Nl)<(1/(Threshold))&(!l%in%ImmundetectionRecord)
...
plot(WtMeanNTime$WTmean~WtMeanNTime$Time,xlim=c(0,30),type='s',xlab='',ylab='',main='NULL',cex.main=3,cex.axis=2,ylim=c(min(LAll),max(LAll)))
...
plot(WtMeanNTime$LengthDom~WtMeanNTime$Time,xlim=c(0,30),type='s',xlab='',ylab='',main='NULL',cex.main=3,cex.axis=2,ylim=c(min(LAll),max(LAll)))
