...
PopuDynamic<-function(t,Nl,l){
        ...
        SwTo<-function(l){return(1/length(LAll)+DST*(l-median(LAll)))}
       ...
       #SwTo<-function(l){return((1+Diffac*(median(LAll)-l))/length(LAll))}#switch preference
        ...
        ImKill<-function(Nl){#immune dection and kill,threshold version 
                if(any(Nl>Threshold&(!(l%in%ImmundetectionRecord)))){
                        DetectionTime<<-c(DetectionTime,rep(t,length(l[Nl>Threshold&(!l%in%ImmundetectionRecord)])))#record detection time
                        ImmundetectionRecord<<-c(ImmundetectionRecord,l[Nl>Threshold&(!l%in%ImmundetectionRecord)])
                        }
                if(any(l%in%ImmundetectionRecord)){ OutputVec<-rep(0,length(l))
                          OutputVec[l%in%ImmundetectionRecord]<-KillingRate
...
#                         ImmundetectionRecord<<-c(ImmundetectionRecord,l[rexp(length(l),rate = Nl)<1e-9])
...
plot((2-DG*(LAll-median(LAll)))~LAll,ylab='Replications/day',xlab='VSG lengths(bp)',type='l',lwd=2,cex.lab=2,cex.axis=2)

#immune detection time vs length 
plot(ImmuneVec$DetectionTime~ImmuneVec$VSGlength,xlab='VSG lengths(bp)',ylab='Time detected by adaptive immune system(days)',cex.lab=2,cex.axis=2)
